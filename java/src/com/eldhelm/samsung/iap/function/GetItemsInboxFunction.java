...
public class GetItemsInboxFunction implements FREFunction {
	...
	public FREObject call(FREContext arg0, FREObject[] arg1) {
		...
		try {
			itemList = FREArray.newArray(FREIapPurchasedItem.CLASS_NAME, 0,
					false);
		} catch (Exception e) {
			...
			if (null != bundle) {
				...
				if (statusCode == 0) {
					...
					for (String itemString : arrayList) {
						itemList.setObjectAt(i++, new FREIapPurchasedItem(
								itemString));
					}
				} else if (statusCode == -1001) {
					frecontext.updatePakcage(bundle);
				} else {
					frecontext.sendError("getItemListInbox " + statusCode);
				}
			} else {
				frecontext.sendError("getItemListInbox bundle is null");
			}
